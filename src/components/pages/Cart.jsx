...
import emptyCartImage from "../../assets/empty-cart.png"; // Import your empty cart image
...
const CartPage = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      <h1 className="text-3xl font-bold mb-4 text-center">Welcome to Your Cart!</h1>
      {message && <p className="bg-yellow-200 text-yellow-800 p-2 rounded mb-4">{message}</p>}

      {cart.length === 0 ? (
        <div className="flex flex-col items-center justify-center min-h-[300px] text-center">
          <img src={emptyCartImage} alt="Empty Cart" className="w-48 h-48 mb-4" />
          <h2 className="text-3xl font-semibold text-gray-600">Your cart is empty</h2>
          <p className="text-lg text-gray-400 mt-2">Add some items to your cart to get started!</p>
        </div>
      ) : (
        <div>
          <ul className="space-y-4">
            {cart.map(item => {
              ...
              return (
                <li key={item.id} className="flex items-center space-x-4 p-4 border border-gray-300 rounded-lg shadow-md hover:shadow-lg transition-all">
          ...
          <button onClick={handleCheckout} className="mt-4 py-2 px-6 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition-all">Checkout</button>
        </div>
      )}
    </div>
  );
