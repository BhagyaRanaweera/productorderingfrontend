...
const FlashSaleItem = ({ item }) => {
  ...
  useEffect(() => {
    ...
    // Ensure saleEndTime is a valid date
    const countdownEnd = new Date(item.saleEndTime); // Assuming item.saleEndTime is a valid date string

    console.log('Countdown end:', countdownEnd); // Log the parsed date to check if it's valid

    if (isNaN(countdownEnd)) {
      console.error('Invalid saleEndTime:', item.saleEndTime);
    ...
    <div className="relative mx-2">
      ...
      <div className="flex md:items-start items-center flex-col">
        ...
        <div className="flex mt-2 text-gray-500 text-sm font-semibold gap-2 items-center">
          ...
          <RatingComp text={i18n.t("productPage.review")} variant="primary" item={item} />
        </div>
        {/* Countdown Timer */}
...
FlashSaleItem.propTypes = {
  item: PropTypes.shape({
    ...
    saleEndTime: PropTypes.string.isRequired, // Add saleEndTime prop
  }).isRequired,
};
